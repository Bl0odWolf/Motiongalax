...
function fade.new(_duration, _mode, _loop)
--@               ^          ^      ^
--@              numb       str    bool
...
function fade:print(_inputStr, _x, _y, _r, _sx, _sy, _ox, _oy)
--@                      ^    ^    ^   ^    ^    ^    ^   ^
--@                     str numb numb numb numb numb numb numb
    ...
    --%function%--
    local _red, _g, _b, _a = love.graphics.getColor()
    love.graphics.setColor(_red, _g, _b, self.alph)
    love.graphics.print(_inputStr, _x, _y, _r, _sx, _sy, _ox, _oy)
    love.graphics.setColor(_red, _g, _b, _a)
end

function fade:update(_elapsed, _speed)
--@                       ^         ^
--@                      numb      numb
    --%error tratament%--
    --#if nil
    ...
    --%function%--
    local tween = 1 / (self.duration * 100) * _speed
    if self.mode == "in" then
        if self.alph < 1 then
            self.alph = self.alph + tween
