...
function resize.new(_duration, _mode, _loop)
    ...
    if _mode == "in" then
        _animetion.sx, _animetion.sy = 0, 0
    end
    return _animetion
end

function resize:print(_inputStr, _x, _y, _r, _sx, _sy, _ox, _oy)
--@                      ^       ^   ^   ^   ^    ^    ^    ^
--@                     str    numb numb numb numb numb numb numb
    ...
    --%function%--
    self.maxSx, self.maxSy = _sx, _sy
    if self.mode == "out" then
        if self.sx == nil and self.sy == nil then
            self.sx, self.sy = _sx, _sy
        end
    end
    love.graphics.print(_inputStr, _x, _y, _r, self.sx, self._sy, _ox, _oy)
end

function resize:update(_elapsed, _speed)
    --%error tratament%--
    --#if nil
    if _speed == nil then
        _speed = 1
    ...
    --%function%--
    local _tweenSx, _tweenSy = self.maxSy / (self.duration * 100) * _speed, self.maxSx / (self.duration * 100) * _speed
    if self.mode == "in" then
        if self.sx < self.maxSx then
            self.sx = self.sx + _tweenSx
        end
        if self.sy < self.maxSy then
            self.sy = self.sy + _tweenSy
        end
        if self.sx >= self.maxSx and self.sy >= self.maxSx and self.loop then 
            self.mode = "out"
        end
    else
        if self.sx > 0 then
        ...
        if self.sx <= 0 and self.sy <= 0 and self.loop then 
            self.mode = "in"
        end
    end
end
